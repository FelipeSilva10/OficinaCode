...
export function IdeScreen({ role, readOnly = false, onBack, projectId }: IdeScreenProps) {
          ...
          if (data && !error) {
            ...
            try {
              if (data.workspace_data) {
                ...
                if (data.workspace_data?.c) {
                  // Formato novo: { c: "<string comprimida>" }
                  raw = JSON.parse(LZString.decompress(data.workspace_data.c) || '{}');
                } else if (typeof data.workspace_data === 'object') {
                  // Formato legado: JSON puro salvo direto no jsonb
  ...
  // ── Fix: workspace_data é jsonb — string comprimida precisa ficar dentro de { c: "..." }
